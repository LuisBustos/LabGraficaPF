	...
	glPushMatrix();
		...
		glPushMatrix();		
			...
			glPushMatrix();
				...
				glTranslatef(0, movKitUp, movKit); // linea que controla el movimiento del coche
				//Pongo aquí la carroceria del carro
				//MODELO JERARQUICO
				kit.GLrender(NULL,_WIRED,1.0);  //_WIRED O _POINTS O _SHADED
				glPushMatrix();
				glTranslatef(-6, -1.0, 7.5);
				glRotatef(giro_volar, 0, 0, 1); //GIRO PARA VOLAR
				glRotatef(giro_llanta,1,0,0);
				llanta.GLrender(NULL,_SHADED,1.0);
				glPopMatrix(); //Llanta frontal derecha		
				glPushMatrix();
				glTranslatef(-6, -1.0, -9.5);
				glRotatef(giro_volar, 0, 0, 1); //GIRO PARA VOLAR
				glRotatef(giro_llanta,1, 0, 0);
				llanta.GLrender(NULL, _SHADED, 1.0);
				glPopMatrix(); //Llanta trasera derecha
				glPushMatrix();
				glTranslatef(6, -1.0, 7.5);
				glRotatef(giro_volar, 0, 0, 1); //GIRO PARA VOLAR
				glRotatef(giro_llanta, 1, 0, 0);
				glRotatef(180,0,1,0);
				llanta.GLrender(NULL, _SHADED, 1.0);
				glPopMatrix(); //Llanta frontal izquierda
				glPushMatrix();
				glTranslatef(6, -1.0, -9.5);
				glRotatef(giro_volar,0,0,1); //GIRO PARA VOLAR
				glRotatef(giro_llanta, 1, 0, 0);
				glRotatef(180, 0, 1, 0);
				llanta.GLrender(NULL, _SHADED, 1.0);
				glPopMatrix(); //Llanta trasera izquierda
	...
	if (g_fanimacion) {
		...
		if (movKit >= -60 && estado == 0) {
			...
			if (estado == 0) {
				estado = 1;
				giro_volar = 90;
			}//GIRA LLANTA Y CAMBIA DE ESTADO
			if (movKitUp <= 45 && estado == 1) {
				movKitUp += .5;
				giro_llanta += 5;
			}//SUBE
			else {
				...
				if (movKit <= 40 && estado == 2) {
					...
					if (movKitUp > 4 && estado == 3) {
						...
						if (estado == 3){ 
							giro_volar = 0;
							estado = 4;
						}//CAMBIA DE ESTADO Y REGRESA
						  //LAS LLANTAS A LA POSICION ORIGINAL
						if (movKit <= 130 && estado == 4) {
							movKit += .5;
							giro_llanta += 5;
						}//AVANZA HASTA LA POSICION FINAL
						else
							estado = -1; //CAMBIA AL ESTADO DE ESPERA
					}
				}
			}		
		}
		if(estado == -1 )
			g_fanimacion ^= true; //ACTIVA EL CIRCUITO DE NUEVO
	}
	glutPostRedisplay();
}
